...
def main():
    ...
    dir_mgr.create_raw_beh_data_out_dir(data_type=EXP_OR_SIM,
                                        out_dir_label=EXP_LABEL,
                                        make_dir=False)

    this_analyses_raw_data_dir = os.path.join(
        dir_mgr.paths.data, 'rawdata', f'{EXP_OR_SIM}', f'{EXP_LABEL}')

    out_proc_data_dir = os.path.join(
        dir_mgr.paths.data, 'processed_data', f'{EXP_OR_SIM}', f'{EXP_LABEL}')
    out_descr_stats_dir = os.path.join(
        dir_mgr.paths.results, 'descr_stats', f'{EXP_OR_SIM}', f'{EXP_LABEL}')
    out_fig_dir = os.path.join(
        dir_mgr.paths.figures, f'{EXP_OR_SIM}', f'{EXP_LABEL}')
    ...
    # Define file names
    part_fn = os.path.join(this_analyses_raw_data_dir,
                           'participants.tsv')
    events_all_subs_fn = os.path.join(
        out_proc_data_dir, 'sub-all_task-th_run-all_beh')
    ...
    ev_file_list = glob.glob(this_analyses_raw_data_dir + '/*/*/sub-*_task-th_beh.tsv')
    ev_file_list.sort()
    ...
    if grp_lvl_stats_df.empty or (
        ...
        descr_stats_whole_sample = DescrStats(
            events_all_subs_df, EXP_OR_SIM, subject='whole_sample', part_fn=part_fn)
        ...
        if EXP_OR_SIM == 'exp':
            ...
            for block_, block_df in events_all_subs_bw.items():
                descr_stats_whole_sample_bw = DescrStats(
                    block_df, EXP_OR_SIM, subject=f'whole_sample_b')
                grp_lvl_stats_bw[
                    block_] = descr_stats_whole_sample_bw.perform_descr_stats()
                grp_stats_object = GroupStats(
                    block_df, EXP_OR_SIM, descr_stats_all_subs_bw[block_])
    ...
    t_wise_bw = {}
    for block_, block_df in events_all_subs_bw.items():
        grp_stats = GroupStats(block_df, EXP_OR_SIM, descr_stats_all_subs_bw[block_])
        ...
        for block_, block_df in events_all_subs_bw.items():
            ...
            for agent, agent_block_df in agentgroupby_object:
                grp_stats_thisagent = GroupStats(
                    agent_block_df, EXP_OR_SIM, descr_stats_all_subs_bw[block_])
                t_wise_bw_agent[agent][
