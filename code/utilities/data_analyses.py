...
class Counts:
    """Methods to compute participant-level one-dimensional (i.e. unconditioned)
    summary_stats from a treasure hunt data set (experimental or simulated).
    ...
    def eval_duration(self):
        ...
        if 'ons' in self.events_df:
            self.counts_df['duration'] = self.events_df['ons'][self.events_df['ons'].last_valid_index()] \
                            - self.events_df['ons'][self.events_df['ons'].first_valid_index()]
        else:
            self.counts_df['duration'] = np.nan

    def count_trials(self):
        """Count number of non-nan trials and started blocks and rounds and add columns"""
        # Count non-na action_vs (excluding 999, i.e. 'escape')
        self.counts_df['n_val_actions'] = self.events_df['action_v'].mask(self.events_df['action_v'] == 'esc', np.nan).count()
        self.counts_df['n_val_blocks'] = 0  # Initialize blocks count, number will increase only if non-nan action exist
        ...
        if self.counts_df.iloc[0]['n_val_actions'] > 0:
            ...
            self.counts_df['n_val_blocks'] = self.events_df['block'][
                self.events_df['action_v'].mask(self.events_df['action_v'] == 'esc', np.nan).last_valid_index()]

            # Loop through blocks
            for block, block_df in self.events_df.groupby(['block']):
                # Check for valid actions in this block
                if block_df['action_v'].mask(block_df['action_v'] == 'esc', np.nan).count() > 0:
                    # Evaluate No of started blocks (excluding 999, i.e. 'escape')
                    self.counts_df['n_val_rounds'] += self.events_df['round'][
                        block_df['action_v'].mask(self.events_df['action_v'] == 'esc', np.nan).last_valid_index()]
    ...
    def count_action(self):
        ...
        if 'drill' in self.events_df['action_type'].values:
            ...
            self.counts_df['p_drills'] = self.counts_df.iloc[0]['n_drills'] / self.counts_df.iloc[0]['n_val_actions']
            self.counts_df['mean_drills'] = np.nanmean(self.events_df['action_type_num'])
            self.counts_df['std_drills'] = np.nanstd(self.events_df['action_type_num'])
        ...
        if 'step' in self.events_df['action_type'].values:
            ...
            self.counts_df['p_steps'] = self.counts_df.iloc[0]['n_steps'] / self.counts_df.iloc[0]['n_val_actions']
        else:
        ...
        for block, block_df in self.events_df.groupby(['block']):
            ...
            if 'drill' in block_df['action_type'].values:
                n_drills_b_df[f'n_drills_b{block}'] = [action_v_counts_per_b['drill']]
                p_drills_b_df[f'mean_drills_b{block}'] = [np.nanmean(block_df['action_type_num'])]

                # Count drills roundwise
                for round_, round_df in block_df.groupby(['round']):
                    action_v_counts_per_br = round_df['action_type'].value_counts()
                    if 'drill' in round_df['action_type'].values:
                        n_drills_br_df[f'n_drills_b{block}_r{round_}'] = [action_v_counts_per_br['drill']]
                    else:
            ...
            if 'step' in block_df['action_type'].values:
                n_steps_b_df[f'n_steps_b{block}'] = [action_v_counts_per_b['step']]

                # Count steps roundwise
                for round_, round_df in block_df.groupby(['round']):
                    action_v_counts_per_br = round_df['action_type'].value_counts()
                    if 'step' in round_df['action_type'].values:
                        n_drills_br_df[f'n_drills_b{block}_r{round_:02d}'] = [action_v_counts_per_br['step']]
                    else:
                        n_drills_br_df[f'n_drills_b{block}_r{round_:02d}'] = [0]
            else:
        ...
        for round_, round_df in self.events_df.groupby(['round']):
            ...
            # Count drills:
            if 'drill' in round_df['action_type'].values:
                n_drills_r_df[f'n_drills_r{round_:02d}'] = [action_v_counts_per_r['drill']]
                p_drills_r_df[f'p_drills_r{round_:02d}'] = [action_v_counts_per_r['drill']] / \
                                                            round_df['action_type'].count()
                drills_over_blocks_cols = [col for col in n_drills_br_df.columns if f'_r{round_:02d}' in col]
                mean_this_round_over_blocks = np.mean(n_drills_br_df[drills_over_blocks_cols].values)
            else:
            ...
            # Count steps:
            if 'step' in round_df['action_type'].values:
                n_steps_r_df[f'n_steps_r{round_:02d}'] = [action_v_counts_per_r['step']]
                p_steps_r_df[f'p_steps_r{round_:02d}'] = [action_v_counts_per_r['step']] / \
                                                          round_df['action_type'].count()
            else:
...
class ConditionalFrequencies:
    ...
    def tr_giv_n_hides(self):
        ...
        n_tr_giv_n_hides_df = pd.DataFrame([n_tr_giv_n_hides.values], columns=cols)
        self.cond_frequ_df = pd.concat([self.cond_frequ_df, n_tr_giv_n_hides_df], axis=1)

        # Relative frequency
        p_tr_giv_n_hides = groupby_n_hides['tr_disc'].mean()  # out: Pandas series with probs grouped by number of hides
        cols = []
        for number in p_tr_giv_n_hides.index:
            cols.append(f'p_tr_giv_hides_{int(number)}')
        p_tr_giv_n_hides_df = pd.DataFrame([p_tr_giv_n_hides.values], columns=cols)
        self.cond_frequ_df = pd.concat([self.cond_frequ_df, p_tr_giv_n_hides_df], axis=1)
    ...
    def p_unv_giv_action_v(self):
        ...
        p_hide_unv_g_drill_df = pd.DataFrame([p_unv_giv_drill.values], columns=cols)
        self.cond_frequ_df = pd.concat([self.cond_frequ_df, p_hide_unv_g_drill_df], axis=1)
    ...
    def p_visible_hide_giv_tr_disc(self):
        ...
        p_was_unv_hide_giv_tr_disc_df = pd.DataFrame([p_was_unv_hide_giv_tr_disc.values], columns=cols)
        self.cond_frequ_df = pd.concat([self.cond_frequ_df, p_was_unv_hide_giv_tr_disc_df], axis=1)
...
class GroupStats(DescrStats):
    ...
    def eval_t_wise_stats(self, groupby) -> object:
        ...
        for trial, trial_df in group_by_trial:
            ...
            n_action_valid = trial_df['action_v'].mask(trial_df['action_v'] == 'esc', np.nan).count()
            t_wise_counts_df.at[trial - 1, 'trial'] = trial
            t_wise_counts_df.at[trial - 1, 'round'] = 1 + np.floor((trial - 1) / 12)
            if 'step' in trial_df['action_type'].values:
                t_wise_counts_df.at[trial - 1, 'n_steps'] = action_type_counts['step']
                t_wise_counts_df.at[trial - 1, 'p_steps'] = action_type_counts['step'] / n_action_valid # TODO: not quite correct ?!?
                test = np.mean(action_type_counts['step'])
                t_wise_counts_df.at[trial - 1, 'mean_drill'] = np.nanmean(trial_df['action_type_num'])
                t_wise_counts_df.at[trial - 1, 'std_drill'] = np.nanstd(trial_df['action_type_num'])
                t_wise_counts_df.at[trial - 1, 'var_drill'] = np.var(trial_df['action_type_num'])

            else:
                t_wise_counts_df.at[trial - 1, 'n_steps'] = 0
                t_wise_counts_df.at[trial - 1, 'p_steps'] = 0
            if 'drill' in trial_df['action_type'].values:
                t_wise_counts_df.at[trial - 1, 'n_drills'] = action_type_counts['drill']
                t_wise_counts_df.at[trial - 1, 'p_drills'] = action_type_counts['drill'] / n_action_valid
            else:
    ...
    def eval_r_wise_stats(self):
        ...
        # Drills per round
        roundwise_drills_col_names = [col for col in self.descr_df.columns if 'n_drills_r' in col]

        dic = {}
        for col in roundwise_drills_col_names:
            dic[int(col[col.find('n_drills_r') + 10:])] = self.descr_df[col].to_numpy()
            ...
            this_r_stats_df['drills_sem'] = this_r_stats_df.loc[0, 'drills_sdt'] / np.sqrt(self.n_subs)
            r_wise_stats_df = r_wise_stats_df.append(this_r_stats_df, ignore_index=True)
    ...
    def perform_group_descr_stats(self, group_by):
        ...
        for group_, group_df in self.events_df.groupby([group_by]):
            ...
            self.events_df = group_df  # Change current event_df being analized

            descr_stats_this_group_df = self.perform_descr_stats()
            # If experimental group --> only one group take whole descr_allsubs datafrage
            if self.dataset == 'exp':
                descr_stats_this_group_df['mean_tr_over_subs'] = np.nanmean(self.descr_df['n_tr'])
                descr_stats_this_group_df['std_tr_over_subs'] = np.nanstd(self.descr_df['n_tr'])
                descr_stats_this_group_df['min_tr_over_subs'] = min(self.descr_df['n_tr'])
                descr_stats_this_group_df['max_tr_over_subs'] = max(self.descr_df['n_tr'])

                # Compute treausures mean over blocks and subs
                tr_per_block_col_names = [col_name for col_name in descr_stats_this_group_df.columns
                                          if 'n_tr_b' in col_name]
                for col in tr_per_block_col_names:
                    descr_stats_this_group_df[f'mean_tr_over_sub_bw_{col}'] = np.nanmean(self.descr_df[col])
                mean_tr_per_blok_col_names = [col_name for col_name in descr_stats_this_group_df.columns
                                              if 'mean_tr_over_sub_bw_' in col_name]
                mean_tr_per_block_df = descr_stats_this_group_df[mean_tr_per_blok_col_names]
                descr_stats_this_group_df['mean_tr_over_b'] = np.nanmean(mean_tr_per_block_df)
                descr_stats_this_group_df['std_tr_over_b'] = np.nanstd(mean_tr_per_block_df)
                descr_stats_this_group_df['min_tr_over_b'] = np.min(mean_tr_per_block_df)
                descr_stats_this_group_df['max_tr_over_b'] = np.max(mean_tr_per_block_df)

                # Compute action choices average across participants
                descr_stats_this_group_df['mean_drills_over_subs'] = np.nanmean(self.descr_df['mean_drills'])
                descr_stats_this_group_df['std_drills_over_subs'] = np.nanstd(self.descr_df['mean_drills'])
                descr_stats_this_group_df['var_drills_over_subs'] = np.var(self.descr_df['mean_drills'])

            # If simulation group, 6 groups, --> take group-specific descr_allsubs
            elif self.dataset == 'sim':
                this_groups_descr_stats_allsubs = self.descr_df.loc[self.descr_df['agent'].str.match(group_)]
                descr_stats_this_group_df['mean_tr_over_subs'] = np.nanmean(this_groups_descr_stats_allsubs['n_tr'])
                descr_stats_this_group_df['std_tr_over_subs'] = np.nanstd(this_groups_descr_stats_allsubs['n_tr'])
                descr_stats_this_group_df['mean_drills_over_subs'] = np.nanmean(this_groups_descr_stats_allsubs['mean_drills'])
                descr_stats_this_group_df['std_drills_over_subs'] = np.nanstd(this_groups_descr_stats_allsubs['mean_drills'])
                descr_stats_this_group_df['var_drills_over_subs'] = np.var(this_groups_descr_stats_allsubs['mean_drills'])

                # Compute means over blocks
                tr_per_block_col_names = [col_name for col_name in this_groups_descr_stats_allsubs.columns
                                          if 'n_tr_b' in col_name]
                n_tr_per_block_df = this_groups_descr_stats_allsubs[tr_per_block_col_names]
                descr_stats_this_group_df['mean_tr_over_b'] = np.nanmean(n_tr_per_block_df.T)
                descr_stats_this_group_df['std_tr_over_b'] = np.nanstd(n_tr_per_block_df.T)

                drills_per_blocks_col_names = [col_name for col_name in this_groups_descr_stats_allsubs.columns
                                               if 'mean_drills_b' in col_name]
                drills_per_block_df = this_groups_descr_stats_allsubs[drills_per_blocks_col_names]
                descr_stats_this_group_df['mean_drills_over_b'] = np.nanmean(drills_per_block_df.T)
                descr_stats_this_group_df['std_drills_over_b'] = np.nanstd(drills_per_block_df.T)

            group_descr_stats_df = group_descr_stats_df.append(descr_stats_this_group_df, ignore_index=True)
    ...
    def perform_grp_lvl_stats(self, group_by):
        ...
        for group_, group_df in self.descr_df.groupby([group_by]):
            ...
            for col_name, col_data in group_df.iteritems():
                ...
                # TODO: better to check for dytpe == string, but don't know how
                this_groups_df[f'{col_name}_mean'] = np.mean(col_data.values)
                this_groups_df[f'{col_name}_std'] = np.std(col_data.values)
                this_groups_df[f'{col_name}_sem'] = this_groups_df.loc[0, f'{col_name}_std'] / np.sqrt(self.n_subs)

            group_lvl_stats_df = group_lvl_stats_df.append(this_groups_df, ignore_index=True)
...
# ---------------------------------------------
#
# class RoundwiseCounts:
#
#     def __init__(self, dataframe):
#         self.action_vs_per_round = None
#         self.tr_per_round_df = None
#
#     def action_v_distr(self):
#         """
#         a_distribution : action_v distribution
#         """
#         self.action_vs_per_round = self.df.groupby(['block', 'round', 'action_v'])
#
#     def which_round_found(self):
#         """Compute number of hides unveiled, when participant stopped drilling"""
#         self.tr_per_round_df = self.df.pivot_table(values="tr_disc", index=['round'], aggfunc=np.sum)

    # Conditional counts (i.e. given the value of another event)
    # ----------------------------------------------------------

    # Number of trials as a function of the round_number

    # Treasure location distribution as a function of unique nodes
...
# unveiled field
    ...
    # def tr_no_hides(self):
    #     """
    #     tr_per_block : number of treasures over no over hides unveiled
    #     """
    #     self.tr_hides_df = self.df.pivot_table(values='tr_disc', index=['n_hides_s3'], aggfunc=np.sum)
