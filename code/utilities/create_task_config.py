...
class TaskConfigurator:
    ...
    n_hides : int
        Number of hiding spots
    s_1 : array_like
        (n_blocks)x(n_rounds)-dimensional array with values for starting positions
    s_3_tr_loc: array_like
        (n_blocks)x(n_rounds)-dimensional array with values for treasure locations
    hides_loc: array_like
        (n_blocks)x(n_hides)-dimensional array with values for hiding spot locations
    ...
    def __init__(self, task_config_dir, n_blocks, n_rounds, dim, n_hides):
        ...
        self.s_1 = np.full((n_blocks, n_rounds), 0)  # current position
        self.s_3_tr_loc = np.full((n_blocks, n_rounds), 0)  # hidden state, treasure location of current round
        self.hides_loc = np.full((n_blocks, self.n_hides), 0)

    def sample_task_configs(self):
        """Sample all task configurations"""
        # Sample hiding spots from a discrete uniform distribution over all nodes (without replacement)
        for block in range(self.n_blocks):
            self.hides_loc[block] = np.random.choice(self.n_nodes, self.n_hides, replace=False)

        # Sample the starting position from a discrete uniform distribution over all nodes
        for block in range(self.n_blocks):
            for round_ in range(self.n_rounds):
                self.s_1[block, round_] = np.random.choice(self.n_nodes, 1)

        # Sample the treasure location from a discrete uniform distribution over the hiding spots
        for block in range(self.n_blocks):
            for round_ in range(self.n_rounds):
    ...
    def return_task_configuration(self):
        """Return task configuration for given task parameters. Task configurations will be loaded from
        task_config_dir if existing for given  task parameters or sampled and saved to task_config_dir if not
        existing for given task parameters """
        ...
        if os.path.exists(self.task_config_dir):
            ...
            for block_ in range(self.n_blocks):
                ...
                config_this_block_df = pd.DataFrame(index=range(0, self.n_rounds))
        ...
        task_configurations = {'s_1': self.s_1, 's_3_tr_loc': self.s_3_tr_loc, 'hides_loc': self.hides_loc}
