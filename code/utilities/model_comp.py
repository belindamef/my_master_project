...
class ModelComps:
    ...
    def __init__(self, working_dir, dim, n_hides):
        ...
        start = time.time()
        s4_perms_fn_pkl = os.path.join(self.working_dir, 'utilities', f's4_perms_dim-{self.dim}_h{self.n_hides}.pkl')
        ...
        start = time.time()
        prior_fn = os.path.join(self.working_dir, 'utilities', f'prior_dim-{self.dim}_h{self.n_hides}.npy')
        ...
        start = time.time()
        lklh_fn = os.path.join(self.working_dir, 'utilities', f'lklh_dim-{self.dim}_h{self.n_hides}.npy')
