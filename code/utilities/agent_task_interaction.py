...
def start_agent_task_interaction(working_dir, output_dir, n_blocks, n_rounds, n_trials,
                                 dim, n_hides, agent_models, task_configs, model_comps):
    ...
    for agent_model in agent_models:
        ...
        for repetition in range(repetitions):
            ...
            for this_block in range(n_blocks):
                print(f'Agent {agent_model} (rep: {repetition} playing block {this_block} ...')
                ...
                agent = Agent(agent=agent_model, bayesian=bayesian, exploring=exploring,
                              model_comps=model_comps, task_object=task)
                ...
                for this_round in range(n_rounds):
                    ...
                    task.s3_c = task_configs['s_3_tr_loc'][this_block, this_round]
                    ...
                    tr_disc_t = np.full(n_trials + 1, np.nan)  # treasure disc
                    n_black_t = np.full(n_trials + 1, np.nan)  # number of black nodes
                    n_grey_t = np.full(n_trials + 1, np.nan)  # number of grey nodes
                    n_blue_t = np.full(n_trials + 1, np.nan)  # number of blue nodes
                    drill_finding_t = np.full(n_trials + 1, np.nan)  # drill finding
                    tr_found_on_hide_t = np.full(n_trials + 1, np.nan)  # whether tr was found on hide
                    zero_sum_denom_t = np.full(n_trials + 1, np.nan)  # whether post belief update denominator sum was zero

                    # Observation, marginal beliefs, valence, decision, action
                    o_t = np.full(n_trials + 1, np.nan)  # observation of treasure discovery
                    marg_s3_b_t = np.full((n_trials + 1, n_nodes), np.nan)  # belief state hiding spots
                    marg_s4_b_t = np.full((n_trials + 1, n_nodes), np.nan)  # belief state hiding spots
                    a_s1_t = np.full(n_trials + 1, np.nan, dtype=object)  # state-dependent action set
                    o_s2_t = np.full(n_trials + 1, np.nan, dtype=object)  # state-dependent action set
                    v_t = np.full(n_trials + 1, np.nan, dtype=object)  # decision valence values in each trial
                    d_t = np.full(n_trials + 1, np.nan)  # agent's decision
                    a_t = np.full(n_trials + 1, np.nan)  # action
                    p_o_giv_o_t = np.full(n_trials + 1, np.nan, dtype=object)  # b-dep posterioir predictive distribution
                    kl_giv_a_o = np.full(n_trials + 1, np.nan, dtype=object)  # b-dep posterioir predictive distribution
                    agent_t = np.full((n_trials + 1, 1), np.nan)  # Recordings for sanity checks

                    # Max belief state computations
                    max_s3_b_value_t = np.full(n_trials + 1, np.nan)
                    max_s3_b_nodes_t = np.full(n_trials + 1, np.nan, dtype=object)
                    shortest_dist_to_max_s3_b_t = np.full(n_trials + 1, np.nan)
                    closest_max_s3_b_nodes_t = np.full(n_trials + 1, np.nan, dtype=object)
                    ...
                    for this_t in range(n_trials):
                        ...
                        s_2_node_color_t[this_t] = task.s2_t  # reflects state at end of the last trial
                        ...
                        tr_disc_t[this_t] = task.tr_disc_t  # Record treasure discovery in this_trial
                        ...
                        if task.tr_disc_t == 1:
                            ...
                            s_2_node_color_t[this_t + 1] = task.s2_t  # reflects state at end of the last trial
                    ...
                    sim_dat_c['closest_max_s3_b_nodes'] = np.full(n_trials + 1, np.nan)
