    ...
    for agent_model in agent_models:
        ...
        for repetition in range(repetitions):
            ...
            for this_block in range(n_blocks):
                print(
                    f"Agent {agent_model} (rep: {repetition} playing block {this_block} ..."
                )
                ...
                for this_round in range(n_rounds):
                    ...
                    for this_t in range(n_trials):
                        ...
                        s_2_node_color_t[
                            this_t
                        ] = task.s2_t  # reflects state at end of the last trial
                        ...
                        tr_disc_t[
                            this_t
                        ] = task.tr_disc_t  # Record treasure discovery in this_trial
                        ...
                        if task.tr_disc_t == 1:
                            ...
                            n_black_t[
                                this_t + 1
                            ] = task.n_black  # number of black nodes
                    ...
                    sim_dat_c["s2_node_color"] = sim_dat_c["s2_node_color"].astype(
                        "object"
                    )
                    ...
                    sim_dat_c["s4_hide_node"] = sim_dat_c["s4_hide_node"].astype(
                        "object"
                    )
                    for trial in range(n_trials + 1):
                    ...
                    sim_dat_c["max_s3_b_nodes"] = sim_dat_c["max_s3_b_nodes"].astype(
                        "object"
                    )
                    for t in range(n_trials + 1):
                    ...
                    sim_dat_c["closest_max_s3_b_nodes"] = sim_dat_c[
                        "closest_max_s3_b_nodes"
                    ].astype("object")
                    for t in range(n_trials + 1):
                        sim_dat_c.at[
                            t, "closest_max_s3_b_nodes"
                    ...
                    sim_dat_c["v"] = sim_dat_c["v"].astype(
                        "object"
                    )  # hiding spots arrays
                    for trial in range(n_trials + 1):
                    ...
                    sim_dat_c["hiding_spots"] = sim_dat_c["hiding_spots"].astype(
                        "object"
                    )  # hiding spots arrays
                    for trial in range(n_trials + 1):
                    ...
                    sim_dat_c["marg_s3_prior"] = sim_dat_c["marg_s3_prior"].astype(
                        "object"
                    )
                    for trial in range(n_trials + 1):
                    ...
                    sim_dat_c["marg_s4_prior"] = sim_dat_c["marg_s4_prior"].astype(
                        "object"
                    )
                    for trial in range(n_trials + 1):
                ...
                print(
                    f"Agent {agent_model} finished block {this_block} in {end-start} sec"
                )
