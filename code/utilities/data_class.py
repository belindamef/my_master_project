...
class Data:
    """
    A class used extract and clean treasure hunt task tabular data from one subject contained in a CSV file.
    """
    ...
    def drop_practice_trials(self):
        ...
        index_exp_start = self.events_df.where(self.events_df['block_type'] == 'experiment').first_valid_index()
        # Drop practice trials if existent
        if self.events_df.iloc[0]['block_type'] == 'practice':
            n_prac_blocks = \
            self.events_df.iloc[self.events_df.where(self.events_df['block_type'] == 'practice').last_valid_index()][
                'block']
            self.events_df.drop(self.events_df.index[0:index_exp_start], axis=0, inplace=True)  # Drop practice block
            self.events_df.loc[:, 'block'] -= n_prac_blocks  # Reset block count to start with 1

    def add_continuous_trial_count(self):
        """Add column with continuous trial counts overall and for each block"""
        ...
        self.events_df.insert(trial_col_index + 1, 'trial_contin', range(1, n_trials + 1))
    ...
    def extract_block_wise_dataframes(self):
        ...
        for block_, block_df in self.events_df.groupby('block'):
            ...
            events_block_drop13.insert(trial_col_index + 1, 'trial_cont', range(1, n_trials + 1))
            self.events_block[block_] = events_block_drop13

    def map_action_types(self):
        """Rename action with verbose expressions and group into action types"""
        self.events_df['action_v'] = self.events_df['action'].replace([0, -self.dim, 1, self.dim, -1, 999],
                                                                      ['drill', 'up', 'right', 'down', 'left', 'esc'])
        self.events_df['action_type'] = self.events_df['action_v'].map({'drill': 'drill',
                                                                        'up': 'step',
                                                                        'down': 'step',
                                                                        'right': 'step',
                                                                        'left': 'step',
                                                                        'esc': 'esc'})
        self.events_df['action_type_num'] = self.events_df['action_type'].map({'drill': 1, 'step': 0})
    ...
    def prep_data(self):
        ...
        self.events_df['s2_node_color'] = self.events_df['s2_node_color'].apply(self.str_to_array)
        self.events_df['s4_hide_node'] = self.events_df['s4_hide_node'].apply(self.str_to_array)
