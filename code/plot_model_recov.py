...
mle_group_averages = mle_df.groupby(
    ["agent", "tau_gen"])[["tau_mle", "lambda_mle"]].agg(["mean", "std"])
...
for i, gen_model in enumerate(agent_models):
    ...
    # x.sort()
    y = mle_group_averages.loc[gen_model]["tau_mle"]["mean"].values
    e = mle_group_averages.loc[gen_model]["tau_mle"]["std"].values
