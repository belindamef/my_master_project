...
The code was written in Python 3.8.5 and requires the following dependencies:
(1) PsychoPy version 2020.2.3 (including all dependencies)
(2) numpy version 1.19.1
(3) pandas version 1.1.4
...
project_dir = os.sep.join(working_dir.split(os.sep)[:4])  # Should be Users/belindame/mindmimo
stimuli_dir = os.path.join(working_dir, 'stimuli')  # stimuli directory
data_dir = os.path.join(project_dir, 'data')  # rawdata output directory
beh_dir = os.path.join(data_dir, 'beh')  # rawdata output directory

if not os.path.exists(beh_dir):  # create if non existent
    os.makedirs(beh_dir)
...
exp_data_dir = os.path.join(beh_dir, f'{exp_label}')  # main data directory
exp_data_ext_dir = os.path.join(beh_dir, f'{exp_label}_ext')  # extended data
...
# Get input whether or not to show instructions
while True:
    try:
...
config_params.config_file_path = os.path.join(working_dir,
                                              'task_config',
                                              f'b-{exp_blocks}_r-{exp_rounds}_t-{exp_trials}',
                                              f'{exp_label}')
...
config_pr_params.config_file_path = os.path.join(working_dir,
                                                 'task_config',
                                                 f'b-{practice_blocks}_r-{practice_rounds}_t-{practice_trials}',
                                                 f'{exp_label}')
config_pr_params.blocks = practice_blocks
config_pr_params.rounds = practice_rounds
...
# ------Initialize components for experiment presentation---------------------
gridsize = 12  # Size of stimuli.grid in cm
cube_size = gridsize / dim  # size of one cube in the stimuli.grid
n_nodes = dim ** 2  # No. of fields in the stimuli.grid
...
# Specify possible key responses
key_list = ["left", "right", "up", "down", "space", "escape"]
...
if run_practice_blocks == "y":
    blocks = practice_blocks + exp_blocks  # Initialize number of total blocks
    run_practice = True
else:
...
for this_block in range(blocks):
    ...
    # ------Prepare Routine "practice blocks" (if selected)--------
    if run_practice:
        # Specify different round and trial numbers for first two, i.e. practice blocks
        if this_block < practice_blocks:
            block_type = "practice"
            rounds = cp.deepcopy(practice_rounds)
            trials = cp.deepcopy(practice_trials)
            if this_block == 0:
                ...
                win.flip()
                event.waitKeys()
            elif 0 < this_block < practice_blocks:
        ...
        # ------Ending Routine "practice blocks"---------
        elif this_block == practice_blocks:  # If this block is first real repetition of Routine "block"
    ...
    # ------Prepare Routine "experimental blocks"------
    if not run_practice:
        #score = 0  # Initialize score count
    ...
    # Initialise blockwise score count
    score_b = 0
    # Initialize dataframe for data of all rounds in current block
    rawdata_b = pd.DataFrame()
    # Sample and record hiding spots
    # hides_loc, s4_hide_node = sample_hiding_spots()
    if run_practice:
    ...
    for this_round in range(rounds):
        ...
        # Sample starting position from discrete uniform distribution
        # start_node, s3_tr_loc = sample_start_n_tr(hides_loc)
        ...
        for this_trial in range(trials):
            ...
            # Record current position (s1), i.e. pos at beginning of round OR at end, i.e. after action last trial
            ...
            # Check for quit; turn off autoDraws and break trial loop
            if this_resp == 'escape':
                stimuli.move_count.autoDraw = False
                stimuli.round_count.autoDraw = False
                stimuli.score_count.autoDraw = False
        ...
        # Create df for "this_round"
        rawdata_c = pd.DataFrame(index=range(0, trials + 1))
        # rawdata_c['sub_id'] = sub_ID
        rawdata_c['ons'] = ons_t
        rawdata_c['dur'] = dur_t
        ...
        if run_practice or \
                (run_practice_blocks == 'y' and this_block > practice_blocks) or \
