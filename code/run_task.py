...
unv_hide_with_tr = False  # If True, hiding spots will be unveiled upon
# treasure discovery
task_configuration_type = 'create_n_save'  # will load config files, if
# experiment name existent
...
if not os.path.exists(events_json_fn):
    events_json_dic = {
        "ons": {
            "LongName": "onset",
            "Description": "event onset time with respect to the "
                           "beginning of the first trial",
            "Units": "seconds"
        },
        "dur": {
            "LongName": "duration",
            "Description": "duration of the event, calculated by "
                           "subtracting onset time "
                           "from the participants reaction time",
            "Units": "seconds"
        },
        "block_type": {
            "LongName": "block type",
            "Description": "the type of block the events belong to. "
                           "it can either be a 'practice' "
                           f"block or a experimental block."
        },
        "block": {
            "LongName": "block number",
            "Description": f"the number of the block the event belongs "
                           f"to (values: 1-{exp_blocks}) "
        },
        "round": {
            "LongName": "round number",
            "Description": f"the number of the round the event belongs "
                           f"to (values: 1-{exp_rounds}); "
                           "events with the same block and round number"
                           " have the same treasure location"
        },
        "trial": {
            "LongName": "trial number",
            "Description": f"the number of the trial the event belongs"
                           f"to (values: 1-{exp_trials}); for each trial "
                           f"there is one move / decision / action"
        },
        "s1_pos": {
            "LongName": "current position (state s1)",
            "Description": "value encoding the trial-specific current "
                           "position in node-notation, which corresponds"
                           " to the directly observable state s1; "
                           f"values: 0-{dim ** 2 - 1}, enumerating the "
                           f"nodes in the {dim}x{dim}-dimensional grid "
                           f"world from left to right and top to bottom, e,"
                           f"g, 0 corresponds to the top left node "
                           f"{dim - 1} corresponds to the top right node "
                           f"and {dim * dim - 1} corresponds to the "
                           f"right bottom node"
        },
        "s2_node_color": {
            "LongName": f"node background colors (state s2)",
            "Description": f"({dim * dim - 1} x 1)-dimensional vector "
                           f"denoting each node's current background color.",
            "Levels:": [
                "0: black",
                "1: grey",
                "2: blue"
        ...
        "s3_tr_loc": {
            ...
            "Description": "value encoding the round-specific location "
                           "of the treasure in node-notation, "
                           f"values: 0-{dim ** 2 - 1} (for detailed "
                           f"description, see 's1_pos')",
        },
        "s4_hide_node": {
            "LongName": "hiding spot nodes (state s4)",
            "Description": f"({dim * dim - 1} x 1)-dimensional vector "
                           f"denoting for each node whether it "
        ...
        "o": {
            "LonName": "observation",
            "Description": "observation the participant makes on the "
        ...
        "a_s1": {
            ...
            "Description": "the set of actions available to the "
                           "participant dependent on their"
                           f"current position. The state dependent "
                           f"action set corresponds to or "
                           f" is a subset of (-{dim}, 1, {dim}, -1), "
                           f"encoding drilling, upward, "
                           f"right, downward or left movements, while "
                           f"limiting possible action "
                           f"choices to those that do not cross the "
                           f"grid world borders in each "
                           f"direction. For instance, if the current "
                           f"position s1 = 0, "
                           f"a_s1 = (0, 1, {dim}) "
        },
        "action": {
            "LongName": "action",
            "Description": "value encoding participants action in "
                           "current trial",
            "Levels": [
                " 0: drilling",
                ...
                "999: escape, i.e. experiment interrupted in trial "
                "the event belongs to"
            ]
        },
        "tr_disc": {
            "LongName": "treasure discovery",
            "Description": "value encoding whether or not the treasure "
                           "was discovered in the current trial.",
            "Levels:": [
        ...
        "drill_finding": {
            ...
            "Description": "value encoding whether or not a hiding spot "
                           "was unveiled in the current trial, "
        ...
        "tr_found_on_blue": {
            ...
            "Description": "value encoding whether the treasure was "
                           "discovered on a blue node",
            "Levels": [
        ...
        "n_black": {
            ...
            "Description": f"value encoding the number of nodes that "
                           f"have a black background color"
                           f"values: 0-{(dim ** 2 - 1) - n_hides}",
        },
        "n_grey": {
            "LongName": "number of grey nodes",
            "Description": f"value encoding the number of nodes that "
                           f"have a grey background color"
                           f"values: 0-{(dim ** 2 - 1) - n_hides}",
        },
        "n_blue": {
            "LongName": "number of blue nodes",
            "Description": f"value encoding the number of nodes that "
                           f"have a blue background color"
                           f"values: 0-{n_hides}",
        },
        "hiding_spots": {
            "Description": f"({n_hides} x 1)-dimensional vector denoting"
                           f" the locations of the hiding spots"
                           f"for each entry, values range from values: "
                           f"0-{dim ** 2 - 1} "
...
if not os.path.exists(participants_fn):
    participants_dic = {
        ...
        "sex": {
            "Description": "sex of the participant as reported by the "
                           "participant",
            "Levels": {
                "male": "male",
                "female": "female",
        ...
        "handedness": {
            "Description": "handedness of the participant as reported by"
                           " the participant",
            "Levels": {
                "left": "left",
                "right": "right",
...
while True:
    try:
        scr_pres = input(
            "Select presentation screen: Full screen (f) / window (w): ")
...
def show_instructions():
    ...
    possible_hides = np.array(
        [22, 6, 4, 15, 24, 19, 13, 12, 11, 18, 17, 14, 5])
    # Extract first n_hides numbers for hiding spots for demonstration
    hides_demo = possible_hides[0:n_hides]
    ...
    for index, hide in enumerate(non_hides_demo):
        non_hides_xy_demo[index] = rowcol_to_xy(
            node_to_rowcol(hide, dim), dim, gridsize)
    ...
    stimuli.instr_top.text = f'Your task is to find the ' \
                             f'treasure in this grid world.'
    ...
    stimuli.instr_center.text = (f'One game has {exp_rounds} rounds.'
                                 f' In every round, your goal is to find'
                                 f' the treasure within {exp_trials}'
                                 f' moves.')
    stimuli.instr_center.draw()
    win.flip()
    event.waitKeys()
    stimuli.instr_top.text = (
        f'The grid world will be displayed in the center, and your'
        f' remaining moves, current round and score on the left.')
    stimuli.instr_top.draw()
    stimuli.grid.draw()
    ...
    stimuli.instr_top.text = (
        f'At the beginning of each round you will start from a new '
        f'position. From there you can move to neighbouring fields.')
    stimuli.grid.draw()
    stimuli.instr_top.draw()
    ...
    stimuli.arrow_right.pos = rowcol_to_xy(
        node_to_rowcol((pos_demo + 1), dim), dim, gridsize)
    stimuli.arrow_left.pos = rowcol_to_xy(
        node_to_rowcol((pos_demo - 1), dim), dim, gridsize)
    stimuli.arrow_up.pos = rowcol_to_xy(
        node_to_rowcol((pos_demo - 5), dim), dim, gridsize)
    stimuli.arrow_down.pos = rowcol_to_xy(
        node_to_rowcol((pos_demo + 5), dim), dim, gridsize)
    ...
    explanation_text = [
        "Once you step on the field where the treasure lies, it will"
        " be revealed.",
    ...
    event.waitKeys()
    stimuli.instr_top.text = (
        f"If you now take a step upwards... \n"
    ...
    pos_xy_demo = rowcol_to_xy(node_to_rowcol(pos_demo, dim), dim, gridsize)
    stimuli.cube.pos = pos_xy_demo
    stimuli.instr_center.text = (
        f'But not all fields are potential treasure locations.')
    stimuli.instr_center.draw()
    stimuli.instr_top.autoDraw = False
    win.flip()
    stimuli.grid.draw()
    stimuli.instr_top.text = (
        f'Only {n_hides} of the {n_nodes} fields re hiding spots. \n')
    stimuli.instr_top.autoDraw = True
    event.waitKeys()
    win.flip()
    event.waitKeys()
    ...
    pages = [
        f"Only {n_hides} of the {n_nodes} fields are hiding spots. \n",
        "The hiding spots you see here are examples. Each game has new"
        " hiding spots. ",
        f'Hiding spots remain the same throughout all 10 rounds of'
        f' one game.',
        "At the beginning of each round the treasure will be hidden"
        " in one of these hiding spots.",
    ...
    for index, example in enumerate(pages):
        ...
        if index >= 3:  # Only draw treasure starting at third flip
    ...
    one_option_version = ("To find out where the hiding spots are "
                          "located, you can use the unveiling option. ")
    two_options_version = (
        "There are two ways to find out where the hiding spots are"
        " located. One is by finding treasures. The same hiding spot"
        " could again harbour a treasure in a following round, as"
        " indicated by a change of the field background color. Another"
        " one is to use the unveil-hiding-spot option")
    ...
    explanation_text = [
        "Consequently, going on hiding spots will increase you chances"
        " to find the treasure",
        "But at the beginning of each game, the locations of hiding"
        " spots are invisible.",
        "This is how you see the grid world at the start.",
        n_options,
        "To unveil hiding spots, you can trade one move for a 'drill'.",
        "Let's assume for example that your current location was a"
        " hiding spot, which you don't know yet.\n",
    ...
    stimuli.instr_top.text = (
        f"So this is where you stand. \n"
        "Press 'space' to drill.")
    stimuli.instr_top.draw()
    ...
    stimuli.instr_top.text = (
        f"The hiding spots will be unveiled and stay visible for the"
        f" rest of the game.")
    stimuli.instr_top.draw()
    hide_stims_demo[hides_demo[3]].draw()
    stimuli.instr_low.autoDraw = True
    ...
    stimuli.instr_center.text = (
        f"Since the treasure will always be hidden at one of these"
        f" hiding spot, finding them will increase your chances to find"
        f" the treasure. \n ")
    ...
    rules_summary = [
        f"To summarize, you will play the treasure hunt game"
        f" {exp_blocks} times and each game has {exp_rounds} rounds.",
        f"In each round, your goal is to ind the treasure within"
        f" {exp_trials} moves.",
        "The treasure will be hidden on one of the hiding spots, which"
        " remain the same throughout all rounds of one game.",
    ...
    stimuli.instr_center.text = (
        f"Before we start the real game, you will play a practice"
        f" game.\n\n Please ask the instructor in case anything is"
        f" unclear. \n\n Or press 'return' when you're ready to start"
        f" the exercise.")
    stimuli.instr_low.autoDraw = False
    win.flip()
    event.waitKeys()
...
def sample_start_n_tr(hides_loc_):
    ...
    # Sample tr location from hiding spots until it's not start position
...
def get_response():
    ...
    while continue_routine:
        ...
        if ((this_resp == 'left' and s1_rowcol[0, 1] == 0)
            ...
            # If part. gives valid response, end routine after current trial
        else:
    ...
    if action == 0:
        ...
        if s4_hide_node[int(s1_node)] == 0:  # If s1 is not a hiding spot
            if s2_node_color[int(s1_node)] == 0:  # If node is (was) black
                ...
                drill_finding = 3  # Drill finding = 3, if drilled on
                # unveiled spot (i.e. not black)
            # Change color to grey (not a hiding spot)
            s2_node_color[int(s1_node)] = 1
            ...
            if s2_node_color[int(s1_node)] == 0:  # If node is (was) black
                ...
                drill_finding = 3  # Drill finding = 3, if drilled on
                # unveiled spot (i.e. not black)
            # Change color to blue (hiding spot)
            s2_node_color[int(s1_node)] = 2
...
for this_block in range(blocks):
    ...
    if run_practice:
        ...
        if this_block < pract_blocks:
            ...
            if this_block == 0:
                ...
                stimuli.instr_center.text = [
                    f"The practice game is shorter than the real game "
                    f"and the score does not count.\n\n "
                    f"Use this game to practice how to move around and "
                    f"drill. \n\n"
                ...
                stimuli.instr_center.text = (
                    f"You finished one exercise game and will now start"
                    f" a new one. \n\n Remember that the hiding spots"
                    f" will change in the new game.\n\n Press 'return'"
                    f" to start the next game.")
                stimuli.instr_center.draw()
                win.flip()
                event.waitKeys()

        # ------Ending Routine "practice "------------------------------
        # If this block is first real repetition of Routine "block"
        elif this_block == pract_blocks:
            # End practice repetitions of Routine "block"
            run_practice = False
            ...
            stimuli.instr_center.text = (
                f"Good Job! You finished the exercise and can now start"
                f" the real game. \n\n Please ask the instructor in"
                f" case you have any questions.\n If everything is"
                f" clear, press 'return' to enter the actual game.")
            stimuli.instr_center.draw()
            win.flip()
            event.waitKeys()
            stimuli.instr_center.text = (
    ...
    if not run_practice:
        ...
        # Initialize this_resp variable for quit ("escape") option
        this_resp = None
    ...
    rawdata_b = pd.DataFrame()  # df for data of all rounds in current block
    ...
    if run_practice:
        ...
        hides_loc = task_configs['hides_loc'][this_block
                                              - (blocks - exp_blocks)]

    s4_hide_node = return_s4_values()
    hides_loc_t = hides_loc  # Record hiding spots
    hides_rowcol = node_to_rowcol(hides_loc, dim)
    s2_node_color = np.full(n_nodes, 0)
    ...
    for this_round in range(n_rounds):
        ...
        tr_disc_t = np.full(trials + 1, np.nan)  # tr discovery in current
        # trial (after action)
        ...
        drill_finding_t = np.full(trials + 1, np.nan)  # finding after
        # drilling (0 = non-hide, 1 = hide)
        tr_found_on_hide_t = np.full(trials + 1, np.nan)  # tr found on unv
        # hide or random

        # Observation, marginal beliefs, valence, decision, action
        o_t = np.full(trials + 1, np.nan)
        a_s1_t = np.full(trials + 1, np.nan, dtype=object)
        a_t = np.full(trials + 1, np.nan)
        ...
        if run_practice:
            ...
            start_node = task_configs['s_1'][this_block
                                             - (blocks - exp_blocks),
                                             this_round]
            s3_tr_loc = task_configs['s_3_tr_loc'][this_block
                                                   - (blocks - exp_blocks),
                                                   this_round]
        ...
        tr_rowcol = node_to_rowcol(s3_tr_loc, dim)
        ...
        for this_trial in range(trials):
            ...
            stimuli.round_count.text = (
                f"Round: {this_round + 1} / {n_rounds}")
            ...
            # ----------------------------------------------------------
            # values at beginning of this round,
            # i.e. at end of last round (after a_{t-1})
            s1_t[this_trial] = s1_node
            s2_node_color_t[this_trial] = s2_node_color
            # --> reflects state at end of last trial
            ...
            if tr_found == 1:
                ...
                n_black_t[this_trial + 1] = np.count_nonzero(
                    s2_node_color == 0)
                n_grey_t[this_trial + 1] = np.count_nonzero(
                    s2_node_color == 1)
                n_blue_t[this_trial + 1] = np.count_nonzero(
                    s2_node_color == 2)
        ...
        rawdata_c['s2_node_color'] = rawdata_c['s2_node_color'].astype(
            'object')
        ...
        # If 'this_round' is NOT forelast or last round,
        # inform participant about new round and score
        if this_round < (n_rounds - 2):
            stimuli.instr_center.text = (
                f"You finished round {this_round + 1} of {n_rounds}."
                f" \n\n There are {n_rounds - (this_round + 1)}"
                f" rounds left in this game.\n\n"
                f"Press any key to continue.")
            stimuli.instr_center.draw()
            win.flip()
            event.waitKeys()

        # If 'this_round' is forelast round,
        # inform participant about new round and score
        elif this_round == (n_rounds - 2):
            stimuli.instr_center.text = (
                f"You finished round {this_round + 1} of {n_rounds}."
                f" \n\n There is {n_rounds - (this_round + 1)} round"
                f" left in this game.\n\n Press any key to continue.")
            stimuli.instr_center.draw()
            win.flip()
            event.waitKeys()

        # If 'this_round' is first or second round in first
        # experimental block, repeat basic rules
        if run_practice or \
            ...
            stimuli.instr_center.text = (
                f"Remember, that the hiding spots will remain the same"
                f" over all rounds but the treasure will be hidden at a"
                f" new spot in every new round \n\n Note that the"
                f" treasure location is drawn at random and thus may"
                f" be located at the same spot more than once .\n\n"
                f"Press any key to continue.")
            stimuli.instr_center.draw()
            win.flip()
            event.waitKeys()
    ...
    # If NOT last block,
    # inform participant about start of new block (alias "game")
    if not run_practice:
        if this_block != blocks - 1:
            stimuli.instr_center.text = (
                f"You completed all rounds of this game. \n\n"
        ...
        # If last block, inform participant about end of entire game
            ...
            stimuli.instr_center.text = (
                f"You finished the last round of the last game \n"
...
# If practice trials exist
if rawdata.iloc[0]['block_type'] == 'practice':
    index_exp_start = rawdata.where(
        rawdata['block_type'] == 'experiment').first_valid_index()
    n_prac_blocks = rawdata.iloc[rawdata.where(
        rawdata['block_type'] == 'practice').last_valid_index()]['block']

    # Create new df that drops practice blocks
    experiment_data = rawdata.drop(rawdata.index[0:index_exp_start], axis=0)
    # Reset block count to start with 1
    experiment_data.loc[:, 'block'] -= n_prac_blocks

    # Write practice data to extra file
    # Create new df that drops exp blocks
    practice_data = rawdata.drop(rawdata.index[index_exp_start:-1], axis=0)
    with open(f'{sub_ext_dir}/practice_events.tsv', 'w') as tsv_file:
        tsv_file.write(practice_data.to_csv(
            sep='\t', na_rep=np.NaN, index=False))
else:
...
with open(f"{sub_beh_dir}/sub-{sub_ID}_task-th_beh.tsv", 'w') as tsv_file:
    tsv_file.write(experiment_data.to_csv(
        sep='\t', na_rep=np.NaN, index=False))
